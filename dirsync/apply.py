...
def apply(
    ...
    # 基于目标目录的实时快照计算差异，保证幂等：
    # 已一致的条目进入 unchanged，只有真正不同的条目才会被写入/删除
    ...
    # 核对 source 里每个待写文件（新建与修改都要核对），
    # 失败则抛 ApplyError，且此时磁盘零改动
    _check_source_files(source_root, manifest, sorted(created + updated))
