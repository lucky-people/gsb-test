...
| `test_miniregex.py` | unittest，73 个用例覆盖全部语义与报错 |
...
| 分组 | 按左括号出现顺序编号，`(?:` 不编号；`\1` 引用该组实际捕获内容；引用不存在的组抛 `PatternError`；反向引用只支持单数字 `\1`-`\9`（`\10` 即 `\1` 后接字面量 `0`），`\0` 抛 `PatternError` |
| 反向引用 | 引用的是该组**最后一次**实际捕获的原文；组未参与匹配（可选组未命中、选择分支落空、前向引用）时反向引用**匹配失败**，注意与"参与匹配但捕获空串"区分（如 `(a*)\1` 在空串上成功） |
...
- 行首/行尾以 `\n` 为界：`multiline=True` 时 `^` 在文本开头或前一个字符为 `\n` 处成立（`\n` 本身的位置不算行首），`$` 在文本末尾或下一个字符为 `\n` 处成立；关闭时 `^`/`$` 只认整串首尾，且默认 `$` **不**在末尾 `\n` 前成立（与 `re` 不同，见上文差异第 1 条）。
- 反向引用在运行时读取当前捕获快照：组未参与匹配（槽位为 `None`）即失败，组参与匹配但捕获空串则按零长度文本比较——两种情形刻意区分，与 `re` 语义一致；回溯会随栈帧恢复捕获内容，重复组上的反向引用引用回溯点处的最后一次捕获。
- 解析器使用 Python 递归：嵌套过深的模式（约千层括号）会触发 `RecursionError`，视为超出设计边界。

## 运行

```bash
python3 -m unittest test_miniregex   # 73 个测试
