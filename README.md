...
从旧实现切换到 lzpack 时，对照暴露出 4 处语义差异。每一处都在
`test_lzpack.py` 的 `TestMigrationRegressions` 中有一条回归用例，
这里记录「为什么这么判定」，避免下次切换再对不上：

1. **字面量块长度 = 标签 + 1**。标签 `0x00..0x7F` 表示 1..128 字节的
   字面量块，解码端按 `标签 + 1` 换算（曾多算一字节变成 `标签 + 2`，
   块边界整体错位）。回归用例手工构造 `标签 0x04 + "hello"` 的流，
   锁定必须恰好解码出 5 字节。
2. **元数据中的 window 上限是 1MB（wlog = 20）**。元数据高 4 位存
   `log2(window) - 10`，合法范围 0..10，即 1KB..1MB。解码端曾按
   `wlog > 19` 拒绝，把合法的 1MB 窗口误判为非法。回归用例锁定
   wlog = 20 必须接受、wlog = 21（2MB）必须抛 `FormatError`。
3. **哈希链前驱表下标必须按 window 取模**。`prev` 是 window 大小的
   环形数组，写入下标为 `i & (window - 1)`；不取模时输入长度超过
   window 会越界崩溃，或从错位槽位取到错误的链前驱。回归用例用
   约 300KB 数据配 1KB 窗口，锁定不崩溃、往返一致且压缩率达标。
4. **window 必须是 2 的幂**。环形数组靠位与取模，非 2 的幂会让取模
   结果错位，因此范围内的非 2 的幂（如 1536、3072）必须抛
   `ConfigError`，不能静默接受。回归用例逐一锁定这些取值。
...
|--------|---------:|---------:|-------:|------------:|------------:|
| 1 MB 随机数据   | 0.588 s | 0.088 s | 100.97% | 2.1 MB  | 3.2 MB  |
| 1 MB 重复数据   | 0.145 s | 0.083 s |   0.16% | 0.4 MB  | 3.0 MB  |
| 10 MB 日志文本  | 1.538 s | 0.846 s |   0.11% | 0.4 MB  | 30.8 MB |
