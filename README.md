...
| `**/temp.tmp` | `**` 可跨任意层目录（含零层，故顶层的 `temp.tmp` 同样匹配） |
...
- `apply` 在修改任何文件**之前**先全量校验清单，并核对 source 里每个
  待写文件（新建与更新都核对）的 size 与 sha256，任一不一致即抛
  `ApplyError`，此时磁盘零改动；
...
- **幂等**：`apply` 基于目标目录的实时快照计算差异，只写入有差异的条目
  （未变化的文件不会被重写）；连续执行两次时，第二次的
  `created` / `updated` / `deleted` 全部为空；
...
diff 四类结果、apply 原子性与幂等（未变化文件不重写）、prune 开关、
dry_run、新建与更新的源文件被改动时都报错且目标零改动、
`**` 匹配顶层文件、verify 检出篡改与多余条目。
