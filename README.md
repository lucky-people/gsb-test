...
- 段内的连续星号（如 `a**b`）按单个 `*` 处理；连续的 `**` 段
  （如 `a/**/**/b`）折叠为一个 `**`。
- 字符类里的 `-` 只在两个字面字符之间才构成区间；位于类首（允许在
  `!`/`^` 之后）、类尾，或写作 `\-` 时按字面连字符处理；区间端点
  也可以用反斜杠转义。下界大于上界的逆序区间（如 `[z-a]`）抛
  `PatternError`。
...
1. **编译期**：每条模式一次性翻译成锚定正则（`^...$`），
   `**` 翻译为 `(?:.*/)?` / `(?:/.*)?`，`*` 翻译为 `[^/]*`，
   字符类逐字符词法分析后再映射（识别 `a-z` 区间、首/尾字面 `]`、
   `\-` 等），类外普通字符统一转义，不会把字母错误地写成正则转义。
   翻译结果是 `re.compile` 后的自动机，匹配单条路径为 O(路径长)，
   无指数回溯。
2. **字面量分桶**：不含任何通配符的模式按
...
- `/` 在模式中总是路径分隔符：字符类不可能跨段，取反字符类
  `[!...]` 也永不匹配 `/`，与 `FNM_PATHNAME` 语义一致；
- 字符类中的 `-` 只在两个字面字符之间解释为区间，类首/类尾与
  `\-` 都是字面连字符，逆序区间直接报 `PatternError`，不做静默纠正；
...
```bash
python3 -m unittest test_pathglob -v   # 71 个用例
...
500 路径 × 50 规则的性能用例，以及字符区间/字面连字符、重复斜杠
折叠、段内 `*` 不跨目录的专项回归（含三入口结论一致性）。
