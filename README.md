...
匹配长度允许大于偏移（重叠匹配）：解压时从「当前输出末尾 − 偏移」
开始**逐字节**拷贝，即最近 `偏移` 个字节循环重复，直到凑满长度。
连续同一字节（偏移 1）、重复日志行（偏移 = 行宽）都依赖这条语义。
...
- **lzpack-f01**（2026-09-26）：修复两条核心语义，各附回归测试：
  1. **重叠匹配拷贝**（`lzpack/codec.py`）：偏移 < 长度 的匹配在解压端
     必须按 LZ77 语义逐字节重复（最近 `偏移` 字节循环），此前用切片
     拷贝会截断，导致连续同字节 / 重复日志行解压错位甚至误报
     `FormatError`；
  2. **变长整数上限**（`lzpack/varint.py`）：`MAX_VARINT_BYTES` 恢复为
     9 字节（63 位），此前被改小为 5，导致 ≥ 6 字节的长整数
     （如 `1 << 40`）编解码不能往返。
