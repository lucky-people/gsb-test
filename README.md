...
- **忽略规则**：按顺序逐条应用，**最后一条命中的规则生效**；`!` 规则把
  之前被忽略的条目重新包含回来。不是「第一条命中就返回」——后面的规则
  （包括 `!` 重新包含）必须有机会覆盖前面的结论；
- **文件是否改动**：`size` 与 `sha256` **同时**一致才算未修改；等长改写
  （大小不变、内容变）必须判为 `modified` / `updated`，只比大小不算数；
- **apply 的差异基准**：以目标目录的**实时快照**为基准计算差异，而不是
  当成空目录重建。已一致的条目进入 `unchanged` 不再重写，这是幂等性的前提；
- **kind 切换**：同一路径 file / dir / symlink 互相切换一律算 `modified`，
  应用时先删旧条目再按新 kind 重建。
...
源文件被改动时报错、verify 检出篡改与多余条目，以及三类对账回归
（忽略规则最后命中生效、等长改写判修改、apply 基于目标现状）。
