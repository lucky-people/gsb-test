...
| `test_miniregex.py` | unittest，67 个用例覆盖全部语义与报错 |
...
| 分组 | 按左括号出现顺序编号，`(?:` 不编号；`\1` 引用该组实际捕获内容；`\0` 非法、引用不存在的组抛 `PatternError`；引用未参与匹配的组则匹配失败 |
...
```bash
python3 -m unittest test_miniregex   # 67 个测试
