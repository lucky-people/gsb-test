...
匹配允许**重叠回引**：匹配长度可以大于偏移。此时按解压顺序逐字节展开，
刚复制出的字节立即成为后续字节的来源（偏移 1 的匹配即 RLE 式游程复制，
连续相同字节、重复日志行都靠它编码）。
...
|--------|---------:|---------:|-------:|------------:|------------:|
| 1 MB 随机数据   | 0.614 s | 0.091 s | 100.97% | 2.1 MB  | 3.3 MB  |
| 1 MB 重复数据   | 0.152 s | 0.087 s |   0.16% | 0.4 MB  | 3.0 MB  |
| 10 MB 日志文本  | 1.627 s | 0.891 s |   0.11% | 0.4 MB  | 30.9 MB |
