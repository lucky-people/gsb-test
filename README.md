...
| 量词 | 闭区间：`{n,m}` 最少 n 次、最多 m 次，`{n,}` 无上界，`{n}` 恰好 n 次；默认贪婪取尽，加 `?` 变懒惰尽量少取——`a{1,3}` 在 `aaaa` 上匹配 `aaa`，`a{1,3}?` 在 `aaa` 上匹配 `a`，`a*?` 匹配空串 |
| 选择 | 分支从左到右尝试，`ab\|a` 在 `ab` 上匹配 `ab` |
| 分组 | 捕获组按左括号出现顺序从 1 开始编号，非捕获组 `(?:...)` 不占编号；`\n` 引用第 n 个捕获组实际捕获的文本，引用不存在的组抛 `PatternError`（`position` 指向反斜杠）；`groups()` 只含捕获组，未参与的组为 `None`，`start(i)`/`end(i)` 对未参与的组返回 -1 |
| 回溯 | 分支回退时捕获槽恢复到进入该分支时的状态；重复结构中同一组多次匹配时，保留最后一次成功迭代的边界（`(a\|b)+c\1` 在 `abcb` 上 `\1` 引用的是 `b`） |
