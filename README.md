...
- 清单路径必须是相对 POSIX 路径：绝对路径、盘符（`C:`）、反斜杠、
  `..`、空路径、重复路径一律抛 `ManifestError`，从根上杜绝目录穿越；
...
- **幂等**：`apply` 先对目标目录做实时快照再计算差异，内容未变的文件
  不会被重写；连续执行两次时，第二次的 `created` / `updated` /
  `deleted` 全部为空；
- `prune=True`（默认）删除目标里清单外的多余条目；`prune=False` 时保留，
  且这些条目不会出现在报告的 `deleted` 列表里。
