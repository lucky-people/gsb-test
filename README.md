...
1. **反向引用指向未参与匹配的组**：该次匹配**失败**（按回溯处理），不是按空串放行。例如 `(a)?b\1` 在 `"b"` 上不匹配，在 `"aba"` 上匹配。
2. **量词计数上限**：`{n,m}` 的 `n`、`m` 均不得超过 10000（含 `{n,}` 的 `n` 与 `{n,m}` 的 `m`），越界在编译期抛 `PatternError`，位置指向 `{`；`a{10000}` 为合法边界。
3. **`.` 与换行**：默认及 `multiline=True` 下 `.` 都不匹配 `\n`；只有 `dot_all=True` 才放行。`multiline` 只影响 `^`/`$`，不影响 `.`。
