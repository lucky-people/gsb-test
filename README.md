...
转义行为说明：含 `\x` 转义的规则（如 `a\*b`、`a\ b`）在编译时会
先去掉转义还原成真实字符，再同时用于正则匹配与 Matcher 的字面量
分桶，因此 `Pattern.matches`、`Matcher.ignores`、`Matcher.match`
三个入口对同一条转义规则永远给出一致结论（曾存在分桶键未去转义
导致 Matcher 静默漏配的缺陷，已修复并有不变量测试兜底）。
