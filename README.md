...
匹配标签之后跟一个变长整数：**偏移 − 1**。取值约定：字段值 `0` 表示
偏移 `1`（重复前一个字节），解码端读出字段后必须 `+1` 还原真实偏移；
因此字段的合法取值是 `0 .. window - 1`，还原后的偏移合法范围是
`1 .. window`（允许等于 window，即引用正好 window 字节之前的历史）。
...
- 窗口上限约束匹配搜索：当前位置 `i` 只沿链访问 `j >= i - window` 的历史
  位置（即偏移 `i - j <= window`，与头部元数据里声明的 window 一致），
  越窗的重复不会成为匹配，只能退化为字面量；`prev` 环形数组的容量正好
  是 window，更老的位置会被覆盖，也不可能被误引用；
