...
对账 / 排查时以以下三条口径为准（均有回归测试锁定，见
`test_dirsync.py` 的 `TestRegressions`）：

- **忽略规则**：规则按列表顺序逐条应用，**最后一条命中**的规则决定
  是否忽略，`!` 前缀规则在其后的位置重新包含；不是「第一条命中即返回」。
- **修改判定**：文件以 `size` + `sha256` 双重判定，**等长改写**
  （大小不变、内容变）也算修改；符号链接比较 `target`；目录只比较
  存在性；`kind` 发生变化（如 file ↔ dir）一律算 `modified`。
- **差异基准**：`apply` 以**目标目录的实时快照**为基准计算
  created / updated / deleted，而不是假设目标为空目录；因此连续两次
  `apply` 幂等，`prune=False` 时目标里多出来的条目原样保留。
...
源文件被改动时报错、verify 检出篡改与多余条目，以及三类对账问题的
定向回归（忽略规则链顺序、等长改写判修改、apply 读取目标现状）。
