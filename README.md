...
- 线上字节里存的是 `偏移 - 1`，解码端读出后必须 `+1` 还原；
  因此字段值 `0` 是合法的，表示偏移 1（源与目标重叠的周期重复）；
- 实际偏移的取值范围是 `1 .. window`（含两端，允许正好等于 window），
  对应字段值 `0 .. window - 1`；window 最大 1 MB，字段最多 3 字节；
- 解压端按头部元数据声明的 window 校验：还原后的偏移若大于 window，
  或大于已解码出的字节数，即 `FormatError`。
...
- 压缩端在位置 `i` 搜索时，只考虑满足 `j >= i - window` 的历史位置
  （即偏移 `i - j <= window`），边界与头部元数据里声明的 window 严格对齐；
- 重复片段的距离一旦超过 window，就只能退化为字面量输出，
  不会发出越窗的匹配——这条不变量同时被压缩端（搜索下界）和
  解压端（偏移校验）双重保证；
- 距离正好等于 window 的重复是合法匹配（边界取闭区间），
  例如 window=32768 时偏移 32768 的匹配必须能编解码。
