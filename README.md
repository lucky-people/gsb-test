...
# 文件按 size + sha256 判修改：等长改写（大小不变、内容变了）也算 modified
...
# 不一致项包括：缺失、多余、内容/大小被篡改、kind 不符（如文件变成目录）
```
...
- `apply` 在修改任何文件**之前**先全量校验清单，并核对 source 里每个
  待写文件（新建与待更新的都核对）的 size 与 sha256，任一不一致即抛
  `ApplyError`，此时磁盘零改动；
...
`**` 匹配零层目录、diff 四类结果与等长改写判定、apply 原子性与幂等、
prune 开关、dry_run、源文件被改动（含等长篡改）时报错且目标零改动、
verify 检出篡改（含等长篡改）、缺失、多余与 kind 不符。
