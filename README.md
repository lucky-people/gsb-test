...
以下三处历史 bug 各有专门的回归测试（`TestRegressions`）：

- `window` 必须是 2 的幂：`validate_config` 拒绝 1023 / 1536 / `3 << 10`
  等非 2 的幂取值，只放行 `1KB..1MB` 之间的 2 的幂；
- 字面量块标签 = 块长 − 1：逐块断言标签字节，并有不可压缩数据的
  端到端往返，防止解码端整块多吃一个字节；
- 哈希链前驱表 `prev` 的下标按窗口取模：输入长度远超 `window` 时
  不越界、链上不取错位历史，匹配偏移始终落在 `1..window` 内。
...
|--------|---------:|---------:|-------:|------------:|------------:|
| 1 MB 随机数据   | 0.580 s | 0.100 s | 100.97% | 2.1 MB  | 3.2 MB  |
| 1 MB 重复数据   | 0.151 s | 0.093 s |   0.16% | 0.4 MB  | 3.0 MB  |
| 10 MB 日志文本  | 1.425 s | 0.942 s |   0.11% | 0.4 MB  | 30.8 MB |
