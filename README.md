...
- 字符类中的 `-` 原样保留以支持区间；位于类开头或结尾的 `-`
  （如 `[-a]`、`[a-]`）与转义的 `\-` 按字面处理，极个别把 `-`
  当字面又放在两字符中间的写法会被解释成区间；
...
```bash
python3 -m unittest test_pathglob -v   # 68 个用例
python3 bench.py                        # 性能基准
```

覆盖：`*` 与 `**` 差异、锚定、目录规则、字符类与取反、转义、
`!` 重新包含与最后匹配优先、路径规范化与非法路径、重复斜杠折叠、
大小写模式、非 ASCII 与空格文件名、4096 字符超长路径、
`!` 规则无前置规则、字符类区间与边界 `-` 的字面语义、
段内 `*` 不跨目录，以及 500 路径 × 50 规则的性能用例。
