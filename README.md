...
1. **编译期**：每条模式一次性翻译成锚定正则（`^...$`），
   中间的 `/**/` 翻译为 `(?:.*/)?`，结尾的 `/**` 翻译为
   `(?:/.*)?`（斜杠可选，因此同时匹配目录自身与下级内容），
   `*` 翻译为 `[^/]*`，字符类直接映射为正则字符类。非锚定规则
   才会加 `(?:.*/)?` 前缀以在任意层级命中；锚定规则（前导 `/`
   或主体中间含 `/`）严格从根开始。目录规则额外捕获命名组
   `inside`：组为空表示命中的是目录自身（要求 `is_dir=True`），
   组非空表示命中的是目录下的内容（与 `is_dir` 无关）。翻译
   结果是 `re.compile` 后的自动机，匹配单条路径为 O(路径长)，
   无指数回溯。
2. **字面量分桶**：不含任何通配符的模式按
...
以及 500 路径 × 50 规则的性能用例。另含三组根因回归：锚定前缀
只作用于非锚定规则、目录自身命中要求 `is_dir=True`、结尾 `/**`
匹配目录自身。
