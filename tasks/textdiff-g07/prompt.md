# 需求单：对齐 textdiff 的对外语义（textdiff-g07）
...
| 2 | 某一侧一行都没有时起始行号多加了 1，文件开头插入这类补丁号错位 |
| 3 | hunk 分组的间隔阈值差一，间隔刚好的两处改动被错误合并 |
| 4 | 冲突块的祖先起始行号少加了 1，替换/删除冲突定位偏一行 |
...
当前 13/42 条失败：
    ...
    ERROR: test_randomized_roundtrip_and_shortest
...
3. 每条现象对应一处独立根因（预计分布在 textdiff/merge.py、textdiff/patch.py），请逐一定位；只改一处会有用例继续红。
