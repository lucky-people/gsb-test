PR 评审意见 —— cronspec：逻辑调整（cronspec-f06）

结论：先别合。基线在 `python3 -m unittest test_cronspec -v` 下已经红 8/35 条，逐条写在下面。

1. @weekly 的星期值与文档不符，短写语义整体偏移一天
2. next_n 传 0 不再抛 ValueError，返回空列表
3. 入参正好命中表达式时 next_after 把入参原样返回，「严格晚于」失效
...
1. 上面每一条我都在本地单独验过，症状各自独立，改一处不会让另一处跟着好。改动面看着分散在 cronspec/engine.py、cronspec/parser.py。
