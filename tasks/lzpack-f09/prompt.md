迁移验收单：把批处理链路从旧实现切到 lzpack（lzpack-f09）

我们准备用 lzpack（纯 Python 标准库实现的 LZ77 压缩库（流格式：魔数 + 版本 + 变长原始长度 + 头部校验 + CRC32 + token 序列））替换现在的旧实现。灰度阶段按老实现的结果做对照，验收用例跑出 17/30 条不一致，切换因此卡在这里。
