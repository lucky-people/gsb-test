迁移验收单：把批处理链路从旧实现切到 textdiff（textdiff-g02）

我们准备用 textdiff（文本差异与三方合并引擎（Myers 最短编辑脚本 + unified diff 生成与套用 + diff3 风格合并））替换现在的旧实现。灰度阶段按老实现的结果做对照，验收用例跑出 6/42 条不一致，切换因此卡在这里。
